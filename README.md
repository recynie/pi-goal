...
Verification appears in the transcript as an expandable, built-in-tool-call-style card. Its title is **Verifying** while the verifier runs, followed by an animated bracketed ASCII `[-]`, `[\]`, `[|]`, `[/]` spinner rendered in subdued, non-bold text. Collapsed output uses a rolling, four-line viewport of the latest trace, and expanded output shows the complete bounded trace, including the verifier request, finalized reasoning and responses, tool calls, and results. Trace labels and text use the ordinary body style.
